//! TODO: Migrate bash_rs to published package.
use std::process;
...
/// Run a shell command and waits for its output.
pub fn run_bash(cmd: String) -> Result<BashOutput, BashError> {
    ...
    match output.status.success() {
        ...
        false => {
            let err = serde_json::to_string_pretty(&BashOutput {
                status: output
                    .status
                    .code()
            ...
            Err(BashError::BashCmdFailed { err })
        }
    }
}

pub fn build_bash_cmd(bash_code: String) -> std::process::Command {
    let mut bash_cmd = std::process::Command::new("bash");
    bash_cmd.arg("-c").arg(bash_code);
    bash_cmd
}

/// run_cmd_print: Runs a command and prints its output. An advantage of this
/// method is that the child proess will continually print to a buffer rather and
/// output will be visible while it's still running.
///
/// This pairs will with a program like "tee" where you want to view logs of a
